...
module.exports = (userDefConfig, defaultOverride) => {
  ...
  const defaultConfig = {
    ...
    whitelistPatternsChildren:
      (currentConfig && currentConfig.whitelistPatternsChildren) || [],
    extensions: (currentConfig && currentConfig.extensions) || [
      '.ts',
      '.html',
      '.js'
    ],
...
function writeConfigFile (config, file) {
  ...
  whitelistPatternsChildren: [${prettyPrintRegex(
    config.whitelistPatternsChildren
  )}],
...
function prettyPrint (list) {
  return list.length
    ? list
      .reduce((prettiness, item) => `${prettiness}\n    '${item}',`, '')
      .slice(0, -1) + '\n  '
    : ''
}
