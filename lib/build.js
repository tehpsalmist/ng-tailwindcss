...
module.exports = ({ purgeFlag, configPath }) => {
  ...
  (Array.isArray(config) ? config : [config]).forEach((bundleConfig, i) => {
    ...
    const sassBuild = (configPath) => new Promise((resolve, reject) => sass.render({
      ...
      const tempFile = configPath
        ? path.normalize(path.resolve(process.cwd(), configPath.replace(/([\w\-.]+[-.][\w\-.]+\.(\w+))/, `temporary-tailwind-css-file-${i}.css`))) // puts temp file in same dir as config file
        : path.normalize(path.resolve(process.cwd(), `temporary-tailwind-css-file-${i}.css`))
