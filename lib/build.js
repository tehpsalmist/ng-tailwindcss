...
module.exports = ({ purgeFlag, configPath }) => {
  const ngTwFile = configPath ? path.normalize(path.resolve(process.cwd(),configPath) ): path.normalize(path.resolve(process.cwd(), 'ng-tailwind.js') ); 
  ...
  const sassBuild = (configPath) => new Promise((resolve, reject) => sass.render({
    ...
    const tempFile = configPath ? path.normalize(path.resolve(process.cwd(),configPath.replace(/([\w\-\.]+[\-\.][\w\-\.]+\.(\w+))/,'temporary-tailwind-css-file.css')), ): path.normalize(path.resolve(process.cwd(), 'temporary-tailwind-css-file.css') )
