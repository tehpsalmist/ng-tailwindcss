  ...
  config.bundles.forEach(bundleConfig => {
    const twBuild = (sourceFile) => new Promise((resolve, reject) => exec(
      ...
      err => {
        ...
        console.info(`Successful Tailwind Build! ${bundleConfig.outputCSS}`)
    ...
    const sassBuild = (configPath) => new Promise((resolve, reject) => sass.render({
      ...
      fs.writeFile(tempFile, result.css, err => err ? reject(err) : console.info(`Sass Compiled for ${bundleConfig.sourceCSS}`) || resolve(tempFile))
    }))
